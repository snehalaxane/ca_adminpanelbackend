    ...
    if (jobCategories.length === 0) {
      const defaultCategory = new JobOpening({
        ...
        jobs: [
          {
            role: 'Senior Auditor',
            shortDescription: 'Experience in statutory audits required',
            description: 'Lead and manage statutory audit engagements for diverse clients.',
...
router.delete("/:categoryId", async (req, res) => {
  try {
    ...
    const deletedCategory = await JobOpening.findByIdAndDelete(categoryId);
...
router.delete("/:categoryId/jobs/:jobId", async (req, res) => {
  try {
    ...
    job.deleteOne(); // safest way
    await category.save();

    res.json(category); // return updated category (important)
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
