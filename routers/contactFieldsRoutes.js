...
const ContactField = require('../model/ContactField');

// Get form fields
router.get('/', async (req, res) => {
  try {
    console.log("GET /api/contact/fields - fetching fields...");
    // Sort by 'order' 
    let fields = await ContactField.find().sort({ order: 1 });
    if (fields.length === 0) {
      fields = [];
    }
    console.log("Returning contact fields:", fields.length);
    res.json(fields);
  } catch (err) {
    console.error(err);
